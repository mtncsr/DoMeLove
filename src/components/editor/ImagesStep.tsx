...
import { formatFileSize, type ProcessedImage } from '../../utils/imageProcessor';
import { saveProcessedImage, deleteMediaWithPreview } from '../../services/mediaService';
...
export function ImagesStep({ templateMeta }: ImagesStepProps) {
  ...
  const handleImageUpload = async (processed: ProcessedImage) => {
    const { metadata } = await saveProcessedImage(projectId, processed);
    updateProject((prevProject) => {
      const currentImages = prevProject.data.images || [];
      if (currentImages.find((img) => img.id === metadata.id)) {
        return prevProject;
      }
      return {
        ...prevProject,
        data: {
          ...prevProject.data,
          images: [...currentImages, metadata],
        },
      };
    });
  };

  const handleMultipleImageUpload = async (images: ProcessedImage[]) => {
  ...
  const handleDeleteImage = async (imageId: string) => {
    await deleteMediaWithPreview(projectId, imageId);
    updateProject((prev) => ({
      ...prev,
      data: {
        ...prev.data,
        images: prev.data.images.filter((img) => img.id !== imageId),
        screens: Object.fromEntries(
          Object.entries(prev.data.screens).map(([screenId, screenData]) => [
            screenId,
            {
              ...screenData,
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 mb-4">
                      {screenImages.map((image) => (
                        <div key={image.id} className="relative group">
                          <ImageThumb projectId={projectId} image={image} className="w-full h-24 object-contain rounded bg-gray-100" />
                          <button
...
function ImageThumb({ projectId, image, className }: { projectId: string; image: ImageData; className?: string }) {
  ...
  const fallback = image.previewDataUrl;
